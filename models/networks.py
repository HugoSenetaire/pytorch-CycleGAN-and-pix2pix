...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[], dim_year=9):
    """Create a generator
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[],output = 1):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        net = NLayerDiscriminator(input_nc, ndf, n_layers=3, norm_layer=norm_layer, output=output)
...
def cal_gradient_penalty(netD, real_data, fake_data, device, type='mixed', constant=1.0, lambda_gp=10.0):
    ...
    if lambda_gp > 0.0:
        ...
        aux = gradients[0].flatten(1)
        gradient_penalty = (((aux + 1e-16).norm(2, dim=1) - constant) ** 2).mean() * lambda_gp        # added eps
        return gradient_penalty, aux
    else:
        return 0.0, None



class ResnetGeneratorBilinear(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    We adapt Torch code and idea from Justin Johnson's neural style transfer project(https://github.com/jcjohnson/fast-neural-style)
    """

    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', input_size = 128, dim_year = 9):
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, output = 1):
        ...
        sequence += [nn.Conv2d(ndf * nf_mult, self.output, kernel_size=kw, stride=1, padding=padw)]  # output 1 channel prediction map
    ...
    def forward(self, input):
        ...
        print("Output Discriminator",output.shape)
        return output
